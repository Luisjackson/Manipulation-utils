    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.JOYBUTTONDOWN:
                ...
                elif event.button == 7: arduino.write(b'H'); print("Ação: botao start") #botao START
                elif event.button == 8: arduino.write(b'M'); print("Ação: Botão home") 
                elif event.button == 9: arduino.write(b'N'); print("Ação: Anal Esquerdo") 
                elif event.button == 10: arduino.write(b'D'); print("Ação: Anal direito") 
        time.sleep(0.05) 
